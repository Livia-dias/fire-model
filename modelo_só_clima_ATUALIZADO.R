...
if(currentTest == "FWI"){
  nomes_colunas=c("Temperature", "RH", "Precipitation", "Wind Speed", "FMC", "DMC", "DC", "HAS_FIRE")
  colunas_teste_treino=c(1,2,3,4,5,6,7,12)
  range_std=c(1:7)
  decimal=","
  separador=";"
}
if(currentTest == "Clima"){
  nomes_colunas=c("Temperature","RH", "Precipitation", "Wind Speed", "HAS_FIRE")
  colunas_teste_treino=c(1,2,3,4,9)
  range_std=c(1:4)
  decimal="."
  separador=","
}
...
if(APA == "cocha") {
  ...
  if(currentTest=="FWI"){
    arquivo_apa_leitura = "modelo_clima\\APA Cocha\\cocha_model-fwi.csv"
  }
  
  if(currentTest=="Clima"){
    arquivo_apa_leitura = "modelo_clima\\APA Cocha\\cocha_model-clima.csv"
  }
  
  arquivo_saida_predicoes=sprintf("modelo_clima\\APA Cocha\\resultados\\%s\\predicoes_cocha.csv", currentTest)
  arquivo_coef_saida = sprintf("modelo_clima\\APA Cocha\\resultados\\%s\\coeficientes_log_reg_cocha.csv", currentTest)
  arquivo_prob_saida = sprintf("modelo_clima\\APA Cocha\\resultados\\%s\\probabilidade_log_reg_cocha.csv", currentTest)
  sumario_apa = sprintf("modelo_clima\\APA Cocha\\resultados\\%s\\modelo_sumario_cocha.csv", currentTest)
  sumario_apa_std = sprintf("modelo_clima\\APA Cocha\\resultados\\%s\\modelo_sumario_std_cocha.csv", currentTest)
  acuracia_apa = sprintf("modelo_clima\\APA Cocha\\resultados\\%s\\modelo_acuracia_cocha.csv", currentTest)
  sumario_aic = sprintf("modelo_clima\\APA Cocha\\resultados\\%s\\best_AIC_sumario_cocha.csv", currentTest)
  sumario_aic_std = sprintf("modelo_clima\\APA Cocha\\resultados\\%s\\best_AIC_sumario_std_cocha.csv", currentTest)
  razões_de_chances= sprintf("modelo_clima\\APA Cocha\\resultados\\%s\\odds_ratio_cocha.csv", currentTest)
}
if(APA == "cavernas") {
  ...
  if(currentTest=="FWI"){
    arquivo_apa_leitura = "modelo_clima\\APA Cavernas\\cavernas_model-fwi.csv"
    nomes_colunas=c("Temperature", "RH", "Precipitation", "Wind Speed", "FMC", "DMC", "DC", "HAS_FIRE")
    colunas_teste_treino=c(1,2,3,4,5,6,7,12)
    range_std=c(1:7)
    decimal=","
  ...
  if(currentTest=="Clima"){
    arquivo_apa_leitura = "modelo_clima\\APA Cavernas\\cavernas_model-clima.csv"
    nomes_colunas=c("Temperature","RH", "Precipitation", "Wind Speed", "HAS_FIRE")
    colunas_teste_treino=c(1,2,3,4,9)
    range_std=c(1:4)
    decimal="."
  ...
  arquivo_saida_predicoes=sprintf("modelo_clima\\APA Cavernas\\resultados\\%s\\predicoes_cavernas.csv", currentTest)
  arquivo_coef_saida = sprintf("modelo_clima\\APA Cavernas\\resultados\\%s\\coeficientes_log_reg_cave.csv", currentTest)
  arquivo_prob_saida = sprintf("modelo_clima\\APA Cavernas\\resultados\\%s\\probabilidade_log_reg_cave.csv", currentTest)
  sumario_apa = sprintf("modelo_clima\\APA Cavernas\\resultados\\%s\\modelo_sumario_cave.csv", currentTest)
  sumario_apa_std = sprintf("modelo_clima\\APA Cavernas\\resultados\\%s\\modelo_sumario_std_cave.csv", currentTest)
  acuracia_apa = sprintf("modelo_clima\\APA Cavernas\\resultados\\%s\\modelo_acuracia_cave.csv", currentTest)
  sumario_aic = sprintf("modelo_clima\\APA Cavernas\\resultados\\%s\\best_AIC_sumario_cave.csv", currentTest)
  sumario_aic_std = sprintf("modelo_clima\\APA Cavernas\\resultados\\%s\\best_AIC_sumario_std_cave.csv", currentTest)
  razões_de_chances= sprintf("modelo_clima\\APA Cavernas\\resultados\\%s\\odds_ratio_cave.csv", currentTest)
}
if(APA == "pandeiros") {
  ...
  if(currentTest=="FWI"){
    arquivo_apa_leitura = "modelo_clima\\APA Pandeiros\\pandeiros_model_fwi.csv"
    nomes_colunas=c("Temperature", "RH", "Precipitation", "Wind Speed", "FMC", "DMC", "DC", "HAS_FIRE")
    colunas_teste_treino=c(1,2,3,4,5,6,7,12)
    range_std=c(1:7)
    decimal=","
  ...
  if(currentTest=="Clima"){
    arquivo_apa_leitura = "modelo_clima\\APA Pandeiros\\pandeiros_model-clima.csv"
    nomes_colunas=c("Temperature","RH", "Precipitation", "Wind Speed", "HAS_FIRE")
    colunas_teste_treino=c(1,2,3,4,9)
    range_std=c(1:4)
    decimal="."
  ...
  arquivo_saida_predicoes=sprintf("modelo_clima\\APA Pandeiros\\resultados\\%s\\predicoes_pandeiros.csv", currentTest)
  arquivo_coef_saida = sprintf("modelo_clima\\APA Pandeiros\\resultados\\%s\\coeficientes_log_reg_pand.csv", currentTest)
  arquivo_prob_saida = sprintf("modelo_clima\\APA Pandeiros\\resultados\\%s\\probabilidade_log_reg_pand.csv", currentTest)
  sumario_apa = sprintf("modelo_clima\\APA Pandeiros\\resultados\\%s\\modelo_sumario_pand.csv", currentTest)
  sumario_apa_std = sprintf("modelo_clima\\APA Pandeiros\\resultados\\%s\\modelo_sumario_std_pand.csv", currentTest)
  acuracia_apa = sprintf("modelo_clima\\APA Pandeiros\\resultados\\%s\\modelo_acuracia_pand.csv", currentTest)
  sumario_aic = sprintf("modelo_clima\\APA Pandeiros\\resultados\\%s\\best_AIC_sumario_pand.csv", currentTest)
  sumario_aic_std = sprintf("modelo_clima\\APA Pandeiros\\resultados\\%s\\best_AIC_sumario_std_pand.csv", currentTest)
  razões_de_chances= sprintf("modelo_clima\\APA Pandeiros\\resultados\\%s\\odds_ratio_pand.csv", currentTest)
}

cavernas_topografic=data.frame(read.csv(arquivo_apa_leitura,dec = decimal, sep = separador))

cavernas_topografic <- cavernas_topografic[,colSums(is.na(cavernas_topografic))<nrow(cavernas_topografic)]

cavernas_topografic=as_tibble(cavernas_topografic)
...
dados_teste_cave_map=dados_teste_cave_map[,colunas_teste_treino]
dados_treino_cave_map=dados_treino_cave_map[,colunas_teste_treino]
...
dados_treino_cave_map_std[range_std]=scale(dados_treino_cave_map_std[range_std])
...
save.image(sprintf("C:\\Queimadas\\R_\\fire-model\\workspaces-clima\\%s",fileName))
