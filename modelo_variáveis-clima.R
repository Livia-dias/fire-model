...
dados_cavernas= data.frame(read.csv("cavernas_model-clima_completa.csv", sep = ","))

dados_cavernas = as_tibble(dados_cavernas)

names(dados_cavernas)

dim(dados_cavernas)

dados_cavernas$precipitation[dados_cavernas$precipitation<0]=0
...
nrow(dados_cavernas)
nrow(dados_treino_cave)
nrow(dados_teste_cave)
