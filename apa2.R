...
KELVIN_OFFSET<-0
FILE_NAME<-"ERA5_rh_2009_2018_Brazil.csv"
...
for (apa_media in apa_medias) {
  ...
  for(hours in days_list){
    ...
    for(point in hours){
      dayValues = c(dayValues,  min(point))
    }
    
    data_frame_temporario = data.frame(t(dayValues))
    nomeLinha = paste("Mínima do dia ", i)
...
write.csv(maxTemperature,"Min RH.csv", row.names = FALSE)