...
if(APA == "cocha") {
  ...
  if(currentTest=="FWI"){
    arquivo_apa_leitura = "MODELO_REFERÊNCIA\\APA Cocha\\modelo_referência_com_FWI_cocha.csv"
  }
  ...
  if(currentTest=="Topografico"){
    arquivo_apa_leitura = "MODELO_REFERÊNCIA\\APA Cocha\\modelo_referência_cocha.csv"
  }
  
  arquivo_saida_predicoes=sprintf("MODELO_REFERÊNCIA\\APA Cocha\\resultados\\%s\\predicoes_cocha.csv", currentTest)
  arquivo_coef_saida = sprintf("MODELO_REFERÊNCIA\\APA Cocha\\resultados\\%s\\coeficientes_log_reg_cocha.csv", currentTest)
  arquivo_prob_saida = sprintf("MODELO_REFERÊNCIA\\APA Cocha\\resultados\\%s\\probabilidade_log_reg_cocha.csv", currentTest)
  sumario_apa = sprintf("MODELO_REFERÊNCIA\\APA Cocha\\resultados\\%s\\modelo_sumario_cocha.csv", currentTest)
  sumario_apa_std = sprintf("MODELO_REFERÊNCIA\\APA Cocha\\resultados\\%s\\modelo_sumario_std_cocha.csv", currentTest)
  acuracia_apa = sprintf("MODELO_REFERÊNCIA\\APA Cocha\\resultados\\%s\\modelo_acuracia_cocha.csv", currentTest)
  sumario_aic = sprintf("MODELO_REFERÊNCIA\\APA Cocha\\resultados\\%s\\best_AIC_sumario_cocha.csv", currentTest)
  sumario_aic_std = sprintf("MODELO_REFERÊNCIA\\APA Cocha\\resultados\\%s\\best_AIC_sumario_std_cocha.csv", currentTest)
  razões_de_chances= sprintf("MODELO_REFERÊNCIA\\APA Cocha\\resultados\\%s\\odds_ratio_cocha.csv", currentTest)
}
if(APA == "cavernas") {
  ...
  if(currentTest=="FWI"){
    arquivo_apa_leitura = "MODELO_REFERÊNCIA\\APA Cavernas\\modelo_referência_com_FWI_cavernas.csv"
  }
  ...
  if(currentTest=="Topografico"){
    arquivo_apa_leitura = "MODELO_REFERÊNCIA\\APA Cavernas\\modelo_referência_cavernas.csv"
  }
  
  arquivo_saida_predicoes=sprintf("MODELO_REFERÊNCIA\\APA Cavernas\\resultados\\%s\\predicoes_cavernas.csv", currentTest)
  arquivo_coef_saida = sprintf("MODELO_REFERÊNCIA\\APA Cavernas\\resultados\\%s\\coeficientes_log_reg_cave.csv", currentTest)
  arquivo_prob_saida = sprintf("MODELO_REFERÊNCIA\\APA Cavernas\\resultados\\%s\\probabilidade_log_reg_cave.csv", currentTest)
  sumario_apa = sprintf("MODELO_REFERÊNCIA\\APA Cavernas\\resultados\\%s\\modelo_sumario_cave.csv", currentTest)
  sumario_apa_std = sprintf("MODELO_REFERÊNCIA\\APA Cavernas\\resultados\\%s\\modelo_sumario_std_cave.csv", currentTest)
  acuracia_apa = sprintf("MODELO_REFERÊNCIA\\APA Cavernas\\resultados\\%s\\modelo_acuracia_cave.csv", currentTest)
  sumario_aic = sprintf("MODELO_REFERÊNCIA\\APA Cavernas\\resultados\\%s\\best_AIC_sumario_cave.csv", currentTest)
  sumario_aic_std = sprintf("MODELO_REFERÊNCIA\\APA Cavernas\\resultados\\%s\\best_AIC_sumario_std_cave.csv", currentTest)
  razões_de_chances= sprintf("MODELO_REFERÊNCIA\\APA Cavernas\\resultados\\%s\\odds_ratio_cave.csv", currentTest)
}
if(APA == "pandeiros") {
  ...
  if(currentTest=="FWI"){
    arquivo_apa_leitura = "MODELO_REFERÊNCIA\\APA Pandeiros\\modelo_referência_com_FWI_pandeiros.csv"
  }
  if(currentTest=="Clima"){
    arquivo_apa_leitura = "MODELO_REFERÊNCIA\\APA Pandeiros\\modelo_referência_com_clima_pandeiros.csv"
  }
  if(currentTest=="Topografico"){
    arquivo_apa_leitura = "MODELO_REFERÊNCIA\\APA Pandeiros\\modelo_referência_pandeiros.csv"
    
  }
  
  arquivo_saida_predicoes=sprintf("MODELO_REFERÊNCIA\\APA Pandeiros\\resultados\\%s\\predicoes_pandeiros.csv", currentTest)
  arquivo_coef_saida = sprintf("MODELO_REFERÊNCIA\\APA Pandeiros\\resultados\\%s\\coeficientes_log_reg_pand.csv", currentTest)
  arquivo_prob_saida = sprintf("MODELO_REFERÊNCIA\\APA Pandeiros\\resultados\\%s\\probabilidade_log_reg_pand.csv", currentTest)
  sumario_apa = sprintf("MODELO_REFERÊNCIA\\APA Pandeiros\\resultados\\%s\\modelo_sumario_pand.csv", currentTest)
  sumario_apa_std = sprintf("MODELO_REFERÊNCIA\\APA Pandeiros\\resultados\\%s\\modelo_sumario_std_pand.csv", currentTest)
  acuracia_apa = sprintf("MODELO_REFERÊNCIA\\APA Pandeiros\\resultados\\%s\\modelo_acuracia_pand.csv", currentTest)
  sumario_aic = sprintf("MODELO_REFERÊNCIA\\APA Pandeiros\\resultados\\%s\\best_AIC_sumario_pand.csv", currentTest)
  sumario_aic_std = sprintf("MODELO_REFERÊNCIA\\APA Pandeiros\\resultados\\%s\\best_AIC_sumario_std_pand.csv", currentTest)
  razões_de_chances= sprintf("MODELO_REFERÊNCIA\\APA Pandeiros\\resultados\\%s\\odds_ratio_pand.csv", currentTest)
}
...
plot(fitted.values(model_AIC), residuals.glm(model_AIC),
     ...
     xlab = "Valores Observados Ajustados", ylab = "Residuais")
abline(0,0)
...
save.image(sprintf("C:\\R\\fire-model\\workspaces-referências\\%s",fileName))
